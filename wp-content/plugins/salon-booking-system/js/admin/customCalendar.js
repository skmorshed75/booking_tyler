...
var DayCalendarHolydays = {
    ...
    "createPopUp":function (status,firstEl,lastEl,els, rule){
        ...
        button.text(status ? ( single ? holidays_rules_locale.block_this_row : holidays_rules_locale.block_these_rows) :  (single ? holidays_rules_locale.unblock_this_row : holidays_rules_locale.unblock_these_rows));
        button.css({
            top: top,
            position:"absolute"
        });
    ...
    "blockSelection":function(){
        DayCalendarHolydays.callAjax('Add',function(data){
            ...
            .text(DayCalendarHolydays.selection.filter(function (e) { return e }).length > 1 ? holidays_rules_locale.unblock_these_rows : holidays_rules_locale.unblock_this_row)
            .off('click')
